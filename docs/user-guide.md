...
- movable clock, session timer, target/range, frame-rate, party, skillbar,
  ...
  character-progression and skill-unlock widgets;
...
not invent title/rank names or expose a progression write. Trainer-visible
skill IDs, character-learned skill IDs, and account-wide unlock IDs remain
separate; the app does not infer names or provide an unlock or activation
action.
...
social, completion, camera, trade, merchant, skill availability, or game UI
state. Chat and texture features remain labelled as requiring another
certified layout or further research.
