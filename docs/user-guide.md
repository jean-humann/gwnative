...
- movable clock, session timer, target/range, frame-rate, party, skillbar,
  ...
  camera/render, trade-offer, UI-frame inventory, NPC-dialog identity,
  merchant item-ID, character-progression, and skill-unlock widgets;
...
numeric UI-frame identity/state/geometry page, plus passive numeric NPC-dialog
body/agent/button identity and selection context, the bounded numeric merchant
item-ID array, and bounded hard-mode availability, level, experience, faction,
and skill-point progression. Merchant window state, identity, prices, quotes,
and transactions are not inferred from that array. Progression does not invent
title/rank names or expose a progression write. Trainer-visible skill IDs,
character-learned skill IDs, and account-wide unlock IDs remain separate; the
app does not infer names or provide an unlock or activation action.
Encoded item, friend, character, guild, and member names; UUIDs; messages; and
announcements are not exposed. UI labels, callbacks, tooltips, dialog text,
generic dialog payloads, and interactions are also excluded. The widgets
cannot change inventory, social, completion, camera, trade, dialog, merchant,
skill availability, or game UI state. Chat and texture features remain
labelled as requiring another certified layout or further research.
...
`chunks/` cache and `mods/` directory remain shared. The exact storage map is
