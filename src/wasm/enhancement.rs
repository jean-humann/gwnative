...
pub(super) const ENHANCEMENT_TRANSFORM_ABI: u32 = 6;
...
const SNAPSHOT_ABI: u32 = 3;
const SNAPSHOT_BYTES: u32 = 2804;
...
mod tests {
    ...
    fn the_manifest_says_what_the_companion_needs_in_the_order_it_was_certified_in() {
        ...
        assert!(
            json.starts_with(r#"{"transformAbi":6,"snapshotAbi":3,"snapshotBytes":2804,"#),
            "the key order changed, and with it the module's hash: {json}",
        );
        assert!(json.contains(r#""configBytes":316,"programId":1,"buildId":38771,"tableSlot":0,"#));
        assert!(json.contains(r#""layoutWords":[5901856,5918104,5912716,5912712,6,"#));

        // Still valid JSON, and still the 79 words the companion expects.
