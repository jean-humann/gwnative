...
impl Instance {
    ...
    ///
    /// Used after cache maintenance so there is no unlocked gap between
    /// deleting the shared cache and opening the store that will use it.
    pub fn downgrade(self) -> Result<Self, String> {
...
/// Share a lock with other readers, while excluding maintenance.
...
mod tests {
    ...
    fn shared_users_coexist_and_exclude_maintenance() {
        ...
        assert!(
            acquire(&path, Duration::ZERO).is_err(),
            "one remaining shared user is enough to keep maintenance out"
        );
        drop(second);

        let maintenance = acquire(&path, Duration::ZERO).expect("exclusive after readers");
        let reader = maintenance.downgrade().expect("exclusive becomes shared");
