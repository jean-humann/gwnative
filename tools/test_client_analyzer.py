...
def module(bodies: list[bytes]) -> bytes:
    ...
    export_section = section(7, vector([name("main") + b"\x00\x01"]))
...
class ClientAnalyzerTests(unittest.TestCase):
    ...
    def test_extracts_only_module_contract_names_from_jspi(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                'Module.canvas; Module["socket"]; WebAssembly.Suspending; '
                "const privateClientCode = 1;"
            )
            report = inspect_jspi(path)
        self.assertEqual(report["moduleProperties"], ["canvas", "socket"])
